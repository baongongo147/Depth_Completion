...
class TransformUtils(object):
    ...
    def _sample_lidar_line(self, depth_map, step=1, row=200):
        """
        Keep depth values in one row, sampled every `step` pixels.
        Other pixels are 0.
        """
        sampled = torch.zeros_like(depth_map)
        height, width = depth_map.shape[-2], depth_map.shape[-1]
        target_row = row if row is not None else height // 2
